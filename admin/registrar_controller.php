<?php
	...
	if(isset($_POST['add'])){
		...
		if($result){
			...
			$_SESSION['error']='Something went wrong adding registrar.';
			echo "4
				<script>
					location.href='registrar.php';
				</script>
	...
	if (isset($_GET['del'])) {
		...
		try {
			...
			$stmt->execute([$id]);
			if ($stmt->rowCount() > 0) {
				$_SESSION['success'] = 'Registrar removed';
				echo "
					<script>
						location.href='registrar.php';
					</script>
				...
				$_SESSION['error'] = 'Registrar not found or already removed';
				echo "
					<script>
						location.href='registrar.php';
					</script>
	...
	if (isset($_GET['on']) && $_GET['on']==1) {
		$id=$_GET['id'];
		try{
			$stmt = $con->prepare("UPDATE `user` SET `status`='1' WHERE id=? AND `type`='registrar' LIMIT 1");
			if(!$stmt->execute([$id])){
				$_SESSION['error']='Something went wrong activating registrar. Please try again.';
				echo "
					<script>
						location.href='registrar.php';
					</script>
	...
	if (isset($_GET['off']) && $_GET['off']==0) {
		$id=$_GET['id'];
		try{
			$stmt = $con->prepare("UPDATE `user` SET `status`='0' WHERE id=? AND `type`='registrar'");
			if(!$stmt->execute([$id])){
				$_SESSION['error']='Something went wrong deactivating registrar. Please try again.';
				echo "
					<script>
						location.href='registrar.php';
					</script>
