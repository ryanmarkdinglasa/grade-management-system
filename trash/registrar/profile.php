<?php
	...
	if (isset($_POST['update-profile'])) {
		...
		$check_email = getrecord('user', ['username'], [$username]);
		if (!empty($check_email) && ($check_email['username']!=$user['username'])) {
	...
	if (isset($_POST['change-pass'])) {
		...
		if($newpassword!==$confirmpassword){
			...
			try {
				...
				$stmt->execute([$newpasswordhash, $user['isPasswordChanged'] + 1, date('Y-m-d H:i:s'), $user['id']]);
			} catch (Exception $e) {
