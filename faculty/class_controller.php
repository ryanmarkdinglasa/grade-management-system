<?php
	...
	if (isset($_GET['del'])) {
		$id = $_GET['id'];
		try {
			$checkClass=findrecord('schedule',['class_id'],[$id]);
			if($checkClass){
				$_SESSION['error'] = 'Class cannot be removed.';
				echo"<script>location.href='class.php';</script>"; 
				exit();
			}
	...
	if (isset($_POST['set-schedule'])) {
		...
		if (empty($days) || empty($time) || empty($room) || empty($course_id)|| empty($class_id) || empty($faculty_id)) {
			$_SESSION['error'] = 'All fields are required.';
			echo"<script> window.location.href='class_schedule.php?id=$class_id'; </script>";
			exit();
		}
