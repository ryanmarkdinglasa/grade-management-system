<?php
	...
	//validate isgraded if already submited a grade, restrict
	...
	$_SESSION['class']=$_GET['class_id'];	//class id
	$_SESSION['period']=$_GET['sched_id'];	// schedule id

	if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['midterm'])) {
		try {
			...
			$hasErrors = false; // Variable to track if there are any errors
			...
			foreach ($grades as $student_id => $grade) {
				...
				// Execute the insert query for each student (assuming you have a unique identifier like student_id)
				$update_sql = "UPDATE `grade` SET `midterm`=:grade, `note`=:note, `updated_on`=:created_on WHERE `student_id`=:student_id AND `schedule_id`=:schedule_id";
				...
				$update_stmt->bindParam(':student_id', $student_id, PDO::PARAM_INT);//  student_id as the unique identifier
				$update_stmt->execute();
			}
	
			if (!$hasErrors) {
				$_SESSION['success'] = 'Midterm grades are submitted.';
				$update_sql = "UPDATE `isgraded` SET `isMidterm`='1', `updated_on`=:created_on WHERE `class_id`=:class_id AND `course_id`=:course_id";
				$update_stmt = $con->prepare($update_sql);
				$update_stmt->bindParam(':created_on', $created_on, PDO::PARAM_STR);
				...
				<div class="col-lg-6 col-7">
				  <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
					...
					<li class="breadcrumb-item"><a href="./"><i class="fas fa-home"></i></a></li>
					  ...
					  <li class="breadcrumb-item active" aria-current="page">Midterm|Grade Sheet Encoding</li>
					</ol>
				  </nav>
				</div>
			...
			<div class="container-fluid mt--6">
				  <div class="row">
					...
					<div class="card-header border-0">
					  <div class="row">
						<div class="col-6">
						  <h3 class="mb-0">Midterm|Grade Sheet Encoding</h3>
						  	<br>
						  	<?php
					...
					<div class="table-responsive">
						...
						<tbody>
					  	...
					  	<div class="text-right">
							...
							<button type="submit" id="midterm" name="midterm" class="add btn btn-primary text-white my-4" style="margin-right:25px;">Submit</button>
						</div>
						</form>
					</div>
				</div>
