<?php
	...
	$count_participants=0; $content_right='';

	//validate class id
	if(!isset($_GET['id']) ){
        echo "<script>window.location.href='403.php';</script>";
        exit();
	}else{ $_SESSION['class']=$_GET['id']; }
									...
									-->
										<td class="text-center">
											...
											<form action="" method="POST">
												...
												<input type="submit" class="btn bg-green text-primary font-weight-bolder btn-sm " name="add-student"  id="add-student"value="+ Student">
											</form>
										...
										<tr>
											...
											<td>
												<span class="text-primary">
													<?php
														$created_on = isset($row['created_on']) ? htmlspecialchars(created_on($row['created_on']), ENT_QUOTES, 'UTF-8') : '';
														echo $created_on; echo formatDate($created_on); ?>"> <?php echo $created_on; ?></label>
												</span>
											</td>
											<td class="text-center">
